...
export default function App() {
  ...
  const addNode = async () => {
    ...
    try {
      // Tell Java to add the node
      const response = await fetch("http://localhost:8080/api/node/add", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        console.log(`⚡ Node ${newId} synchronized with Java.`);
      }
    } catch (err) {
      console.error("Sync failed. Is the Java server running?", err);
    }
  };
...
// --- STYLES ---
const containerStyle = { display: "flex", flexDirection: "column", background: "#0b1220", height: "100vh", color: "#e5e7eb", fontFamily: 'sans-serif' };
const headerStyle = { padding: '15px 20px', borderBottom: '1px solid #1f2937', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#0f172a' };
const toolbarStyle = { position: 'absolute', top: 30, left: 30, zIndex: 10 };
const btnStyle = { padding: '10px 16px', borderRadius: '6px', border: 'none', background: '#3b82f6', color: '#fff', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 6px rgba(0,0,0,0.3)' };
const sidebarStyle = { width: "280px", padding: "20px", background: "#0f172a", borderLeft: '1px solid #1f2937' };
const footerStyle = { padding: '8px 20px', fontSize: '11px', color: '#64748b', background: '#020617', borderTop: '1px solid #1f2937' };
const statusCardStyle = { marginTop: '20px', padding: '15px', background: '#1e293b', borderRadius: '8px', border: '1px solid #334155' };

function Metric({ label, value }) {
  return (
    <div style={{ marginBottom: '12px', padding: '12px', backgroundColor: '#020617', borderRadius: '8px', border: '1px solid #1e293b' }}>
